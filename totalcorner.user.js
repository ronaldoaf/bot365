	...
	setInterval(function(){
		...
		$('.minutes_postfix').parents('tr').each(function(i,e){ 
			...
			DATA.push( {
				...
				daH: Number($(e).find('.match_attach').text().split(' - ')[0]),
				daA: Number($(e).find('.match_attach').text().split(' - ')[1]),
				soH: Number($(e).find('.match_on').text().split(' - ')[0]),
				soA: Number($(e).find('.match_on').text().split(' - ')[1]),
				sfH: Number($(e).find('.match_off').text().split(' - ')[0]),
				sfA: Number($(e).find('.match_odff').text().split(' - ')[1])		
