...
localStorage.betslip_clicado= localStorage.betslip_clicado || false
		   ...
		   $(jogos).each(function(ii,jogo){
		         ...
		         if(  (jogo.home==home) && (jogo.away==away) ){
					   ...
					   jogo_selecionado=bot.jogoLive(home,away);
                        ...
                        //Aposta no Away
					    if (
							 ( ( jogo.ind<=-3.50 ) &&  ( jogo.ind2<=-2.50) && 	( ahSel(jogo_selecionado.selAway)==-0.5)  &&  ( jogo.gA<=0.0) )  ||
...
unsafeWindow.setInterval(function(){
    ...
    if(Number($('#betslipBarEnhancedSelectionCount').text())>=2) {
		if ( !localStorage.betslip_clicado ) {
